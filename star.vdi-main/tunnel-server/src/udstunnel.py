...
from vdi_tunnel import config, proxy, consts, processes, stats
...
def tunnel_main(args: 'argparse.Namespace') -> None:
    ...
    # We will not reuse port, we only want a VDI tunnel server running on a port
    ...
    try:
        ...
        if setproctitle:
            setproctitle.setproctitle(f'VDITunnel {cfg.listen_address}:{cfg.listen_port}')
