...
/**
 * Service which communicates with the remote OpenVDI connection service,
...
public class ConnectionService {
    ...
    /**
     * The name of the parameter returned by the OpenVDI connection
    ...
    /**
     * Makes an HTTP GET request to the OpenVDI service running at the given
     ...
     * by a tab character. If the OpenVDI service encounters an error, it is
     * expected to return the single word "ERROR" on one line. Lines which do
     * not match these expectations will be skipped.
     *
     * @param uri
     *     The URI of the OpenVDI service to which the HTTP GET request should
     *     be made.
     *
     * @return
     *     A map of all parameter name/value pairs returned by the OpenVDI
     *     service.
     *
     * @throws GuacamoleException
     *     If the OpenVDI service returns an error, or the response from the
    ...
    private Map<String, String> readConnectionConfiguration(URI uri)
        ...
        try {
            ...
            throw new GuacamoleServerException("Unable to open connection to OpenVDI service.", e);
        }
        ...
        try {
            ...
            while ((inputLine = response.readLine()) != null) {
                ...
                if (inputLine.equals("ERROR"))
                    throw new GuacamoleServerException("OpenVDI service returned an error.");
        ...
        // Rethrow any error which occurs during reading
            ...
            throw new GuacamoleServerException("Failed to read response from OpenVDI service.", e);
        }
        ...
        // Always close the stream
            ...
            try {
                ...
                logger.warn("Closure of connection to OpenVDI failed. Resource may leak.", e);
            }
    ...
    /**
     * Queries OpenVDI for the connection configuration for the connection
     * associated with the given data. This data is an opaque value provided
     * via the "data" parameter to the Guacamole tunnel.
     *
     * @param data
     *     The OpenVDI-specific data which defines the connection whose
     *     configuration should be retrieved.
     *
     * @return
     *     The configuration of the connection associated with the provided
     *     OpenVDI-specific data.
     *
     * @throws GuacamoleException
     *     If the connection configuration could not be retrieved from OpenVDI,
     *     of the response from OpenVDI was missing required information.
     */
    public GuacamoleConfiguration getConnectionConfiguration(String data)
        ...
        URI serviceURI = UriBuilder.fromUri(configService.getVDIBaseURI())
                .path(data)
                .build();
        ...
        if (protocol == null)
            throw new GuacamoleServerException("Protocol missing from OpenVDI response.");
