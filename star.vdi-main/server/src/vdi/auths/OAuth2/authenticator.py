...
from vdi.models.util import getSqlDatetime
from vdi.core import auths, exceptions
from vdi.core.ui import gui
from vdi.core.util import model, auth as auth_utils
...
class TokenInfo:
    ...
    def from_dict(dct: dict[str, typing.Any]) -> 'TokenInfo':
        ...
        return TokenInfo(
            ...
            expires=getSqlDatetime() + datetime.timedelta(seconds=dct['expires_in'] - 10),
...
class OAuth2Authenticator(auths.Authenticator):
    ...
    clientId = gui.TextField(
        ...
        label=_('Client ID'),
        order=2,
        tooltip=_('Obtained from App created on Azure for VDI Enterprise'),
        required=True,
        tab=_('Server'),
    )
    clientSecret = gui.TextField(
        ...
        order=3,
        tooltip=_('Obtained from App created on Azure for VDI Enteprise - Keys'),
        required=True,
        tab=_('Server'),
    )
    scope = gui.TextField(
    ...
    redirectionEndpoint = gui.TextField(
        ...
        order=90,
        tooltip=_('Redirection endpoint for OAuth2.  (Filled by VDI)'),
        required=False,
        tab=gui.ADVANCED_TAB,
    )
    responseType = gui.TextField(
    ...
    def  _request_token(self, code: str, code_verifier: typing.Optional[str] = None) -> TokenInfo:
        ...
        req = requests.post(self.tokenEndpoint.value, data=param_dict, timeout=30,verify=False)
    ...
    def initialize(self, values: typing.Optional[dict[str, typing.Any]]) -> None:
        ...
        if self.responseType.value in ('code', 'pkce', 'openid+code'):
            if self.commonGroups.value.strip() == '':
                raise exceptions.ValidationError(
                    gettext('Common groups is required for "code" response з')
                )
        ...
        if self.redirectionEndpoint.value.strip() == '':
    ...
    def logout(
        self,
        request: 'ExtendedHttpRequest', username: str
    ) -> typing.Optional[str]:
        return None
    ...
    def getGroups(self, username: str, groups_manager: 'auths.GroupsManager') -> None:
    ...
    def auth_callback_code(
        ...
        # Get the code
        code = parameters["get_data"].get('code', '')
        if code == '':
    ...
    def auth_callback_token(
        ...
        token = TokenInfo(
            access_token=parameters["get_data"].get('access_token', ''),
            token_type=parameters["get_data"].get('token_type', ''),
            expires=getSqlDatetime()
            + datetime.timedelta(seconds=int(parameters["get_data"].get('expires_in', 0))),
            refresh_token=parameters["get_data"].get('refresh_token', ''),
            scope=parameters["get_data"].get('scope', ''),
            info={},
            id_token=None,
        )
        return self._process_token(self._request_info(token), gm)
